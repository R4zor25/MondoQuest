<template>
  ...
  <div v-if="!storyFinished"  class="container">
    <Transition name="slide-fade">
      <div class="question-card" v-show="!loading">
        ...
        <h5>Kérdések száma: {{question.currentQuestionNumber + 1}} / {{ question.totalNumberOfQuestions }}</h5>
  ...
  <div v-if="storyFinished" class="centered-container">
    <div class="card">
      <h2>Gratulálok, végére értél a story módnak.</h2>
      <p>Végső pontszámod: {{ this.userPoint }} (ez változhat még kis mértékben a kiértékelésktől függően)</p>
      <p>Ha szeretnél tovább játszani, akkor azt megteheted a végtelen módban.</p>
    </div>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    this.base64Image = '';
          ...
          if(temp.id === 0 && temp.title === "" && temp.description === "" && temp.answers.length === 0){
             store.state.user.user.finishedStory = true;
    ...
    submitAnswer() {
      ...
      axios.post(`https://mondo-quest.fly.dev/api/question/${this.question.id}/story/answer/${this.userId}`, payload, {
        ...
        .then(response => {
          ...
          store.state.user.user.storyPoints = response.data.userPoint;
    ...
    getUserProgress(){
      ...
      axios
        ...
        .then((response) => {
          this.userPoint = response.data.storyPoints;
          this.storyFinished = response.data.finishedStory;
        })
        .catch((error) => {
          error
  ...
  mounted() {
    ...
    if(store.getters.getUser.user.finishedStory === true){
      this.storyFinished = true
    } else {
    this.fetchQuestion();
    }
    this.userPoint = store.getters.getUser.user.storyPoints;
